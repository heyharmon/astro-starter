...
3. **Reference-based work** → When the user provides a reference URL to replicate or draw inspiration from, the root orchestrator MUST do visual capture before delegating to any agent:
   1. **Screenshot the reference** — Use the browser skill (`playwright-cli`) to take a full-page screenshot of the reference URL. This captures layout, imagery, visual weight, and spatial relationships that text extraction misses.
